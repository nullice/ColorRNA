...
//对已经归一化的 RGB 值进行  Gamma 2.2 的变换，
    ...
    ColorRNA.prototype._adt_adaptation = function (lightName, algName)
    {
        ...
        var X = this.xyz.X * this._adt_mtxAdaptMa[0][0] + this.xyz.Y * this._adt_mtxAdaptMa[1][0] + this.xyz.Z * this._adt_mtxAdaptMa[2][0];
        var Y = this.xyz.X * this._adt_mtxAdaptMa[0][1] + this.xyz.Y * this._adt_mtxAdaptMa[1][1] + this.xyz.Z * this._adt_mtxAdaptMa[2][1];
        var Z = this.xyz.X * this._adt_mtxAdaptMa[0][2] + this.xyz.Y * this._adt_mtxAdaptMa[1][2] + this.xyz.Z * this._adt_mtxAdaptMa[2][2];

        X *= (Ad / As);
        Y *= (Bd / Bs);
        Z *= (Cd / Cs);

        this.xyz.X = X * this._adt_mtxAdaptMaI[0][0] + Y * this._adt_mtxAdaptMaI[1][0] + Z * this._adt_mtxAdaptMaI[2][0];
        this.xyz.Y = X * this._adt_mtxAdaptMaI[0][1] + Y * this._adt_mtxAdaptMaI[1][1] + Z * this._adt_mtxAdaptMaI[2][1];
        this.xyz.Z = X * this._adt_mtxAdaptMaI[0][2] + Y * this._adt_mtxAdaptMaI[1][2] + Z * this._adt_mtxAdaptMaI[2][2];

    }

    ColorRNA.prototype._adt_invAdaptation = function (lightName, algName)
    {
        ...
        var X1 = this.xyz.X * this._adt_mtxAdaptMa[0][0] + this.xyz.Y * this._adt_mtxAdaptMa[1][0] + this.xyz.Z * this._adt_mtxAdaptMa[2][0];
        var Y1 = this.xyz.X * this._adt_mtxAdaptMa[0][1] + this.xyz.Y * this._adt_mtxAdaptMa[1][1] + this.xyz.Z * this._adt_mtxAdaptMa[2][1];
        var Z1 = this.xyz.X * this._adt_mtxAdaptMa[0][2] + this.xyz.Y * this._adt_mtxAdaptMa[1][2] + this.xyz.Z * this._adt_mtxAdaptMa[2][2];
...
//让经过 Gamma  的变换 RGB 归一化值还原
    ...
    ColorRNA.prototype.RGB_to_XYZ = function ()
    {
        ...
        {
            this._adt_adaptation("D55", "vonKries");
        }
    ...
    ColorRNA.prototype.XYZ_to_RGB = function ()
    {
        var nucleotids = this._getRGBnucleotids("AdobeRGB", true);
        var xyzs = [this.xyz.X, this.xyz.Y, this.xyz.Z];

        if (this._doAdapta == true)
        {
            xyzs = this._adt_invAdaptation("D55", "vonKries");
        }
...
test_color = new ColorRNA(112, 123, 0);

var test_color2 = new ColorRNA(77, 77, 0);
test_color._dLV = 1;
...
console.log("=========XYZ_to_RGB===========");
console.log(test_color.XYZ_to_RGB());
...
for (var rr = 0; rr < 3; rr++)
{
    for (var gg = 0; gg < 3; gg++)
    {
        for (var bb = 0; bb < 3; bb++)
        {
            ...
            count++;
            test_color._doAdapta = false;
            test_color._dLV = 1;
            test_color.r = rr;
            test_color.g = gg;
            test_color.b = bb;
            test_color.RGB_to_XYZ();
            rgb = test_color.XYZ_to_RGB();
